...
export function Layout() {
  ...
  return (
    <div className="flex h-screen bg-[#F8FAFC]">
      {/* Sidebar */}
      <aside className="w-20 border-r border-[#E4EEF0] flex flex-col items-center py-8 bg-white hidden md:flex">
        <div className="mb-12">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        {/* Header */}
        <header className="h-20 flex items-center justify-between px-12 bg-transparent z-[100]">
          <div className="flex items-center space-x-2 text-xs text-[#075056]/40 uppercase tracking-widest font-bold">
            <Home size={14} />
            <span>Home</span>
            <span className="opacity-20 mx-2">/</span>
            <span className="text-[#075056]">{location.pathname === "/" ? "Dashboard" : location.pathname.split("/")[1]}</span>
          </div>
          ...
          <div className="flex items-center space-x-8">
            ...
            <Link to="/vendors" className="flex items-center space-x-2 text-xs text-[#075056]/60 bg-white px-4 py-2 rounded-full border border-[#E4EEF0] hover:border-[#075056] transition-colors">
            ...
            <div className="flex items-center space-x-4 relative">
              <button 
                ...
                className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors ${showNotifications ? 'bg-[#075056] text-white' : 'text-[#075056] hover:bg-white'}`}
              >
              ...
              <AnimatePresence>
                {showNotifications && (
                  <motion.div 
                    ...
                    className="absolute top-full right-0 mt-4 w-80 bg-white border border-[#E4EEF0] rounded-[32px] p-6 shadow-xl z-[110] space-y-4"
                  >
                    <h4 className="text-sm font-medium text-[#075056]">System Alerts</h4>
                    <div className="space-y-3">
                      {[
                        ...
                        <div key={i} className="p-3 bg-[#F8FAFC] rounded-2xl text-[10px] text-[#075056]/60 leading-relaxed border border-[#E4EEF0]">
                          {n}
                        </div>
                      ))}
                    </div>
              ...
              <div className="relative">
                <div 
                  ...
                  className={`w-10 h-10 rounded-full overflow-hidden border cursor-pointer hover:border-[#075056] transition-all ${showProfile ? 'border-[#075056] ring-2 ring-[#075056]/10' : 'border-[#E4EEF0]'}`}
                >
                ...
                <AnimatePresence>
                  {showProfile && (
                    <motion.div 
                      initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }}
                      className="absolute top-full right-0 mt-4 w-48 bg-[#075056] text-white rounded-[24px] p-2 shadow-xl z-[110]"
                    >
                      <Link to="/settings" onClick={() => setShowProfile(false)} className="block px-4 py-3 text-xs hover:bg-white/10 rounded-xl">Control Center</Link>
                      <Link to="/settings" onClick={() => setShowProfile(false)} className="block px-4 py-3 text-xs hover:bg-white/10 rounded-xl">Surveillance Logs</Link>
                      <div className="h-[1px] bg-white/10 my-1 mx-2"></div>
                      <button className="w-full text-left px-4 py-3 text-xs text-[#FF5B04] hover:bg-white/5 rounded-xl">Log Out</button>
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
