  ...
  return (
    <div className="max-w-7xl mx-auto space-y-12 pb-32 px-4 md:px-8 lg:px-12">
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between px-2">
          <div className="flex items-center space-x-3">
            ...
            <h2 className="text-xl text-[#075056]">Global Node Surveillance</h2>
          </div>
          <div className="flex items-center space-x-4">
            <div className="flex items-center space-x-2 px-4 py-2 bg-emerald-50 rounded-xl border border-emerald-100">
              ...
              <span className="text-[10px] font-bold text-emerald-700">98.4% UPTIME</span>
            </div>
          </div>
        </div>
        <GeoMap />
      ...
      <div className="grid grid-cols-12 gap-8">
        {/* Risk Forecaster */}
        <div className="col-span-12 lg:col-span-7 bg-white border border-[#E4EEF0] rounded-[40px] p-10 space-y-10">
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              <h3 className="text-lg text-[#075056]">Network Growth Projection</h3>
              <p className="text-xs text-[#075056]/40">Projected vs Actual node distribution for Q2 2026.</p>
            </div>
            <div className="flex space-x-2 bg-[#F8FAFC] p-1.5 rounded-xl">
              {['Weekly', 'Monthly'].map(t => (
                <button 
                  key={t}
                  onClick={() => setReportType(t as any)}
                  className={`px-4 py-1.5 rounded-lg text-[10px] font-bold transition-all ${reportType === t ? 'bg-white shadow-sm text-[#075056]' : 'text-[#075056]/30'}`}
                >
                  {t}
                </button>
              ))}
          ...
          <div className="grid grid-cols-3 gap-6 pt-6 border-t border-[#F1F5F9]">
            {[
              { label: 'Forecast Accuracy', val: '94.2%', color: 'text-[#075056]' },
              { label: 'Signal Strength', val: 'High', color: 'text-emerald-600' },
              { label: 'Latency (Avg)', val: '142ms', color: 'text-[#075056]' }
            ].map(stat => (
              <div key={stat.label} className="space-y-1">
                <p className="text-[9px] uppercase tracking-widest text-[#075056]/30 font-bold">{stat.label}</p>
                <p className={`text-lg font-medium ${stat.color}`}>{stat.val}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="col-span-12 lg:col-span-5 bg-[#075056] rounded-[40px] p-10 space-y-10 text-white overflow-hidden relative">
          <div className="absolute top-0 right-0 p-10 opacity-5">
            <Target size={200} />
          </div>
          
          <div className="relative z-10 space-y-1">
            <h3 className="text-lg font-light">Sector Intensity</h3>
            <p className="text-xs text-white/40">Market penetration by industry category.</p>
          </div>

          <div className="h-64 w-full relative z-10">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={SECTOR_INTENSITY}>
                ...
                <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fontSize: 9, fill: 'rgba(255,255,255,0.4)' }} dy={10} />
                <Tooltip 
                  cursor={{ fill: 'rgba(255,255,255,0.05)' }}
                  contentStyle={{ backgroundColor: '#075056', borderRadius: '15px', border: '1px solid rgba(255,255,255,0.1)', color: '#fff' }}
                />
              </BarChart>
            </ResponsiveContainer>
          </div>
            ...
            {SECTOR_INTENSITY.slice(0, 3).map(sector => (
              <div key={sector.name} className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/10">
                <div className="flex items-center space-x-3">
                  ...
                  <span className="text-xs font-bold uppercase tracking-widest">{sector.name}</span>
                </div>
                <div className="flex items-center space-x-4">
      ...
      <div className="space-y-8">
        <div className="flex items-center space-x-3 px-2">
          ...
          <h2 className="text-xl text-[#075056]">System Report Library</h2>
        </div>
          ...
          {systemReports.map((report, i) => (
            ...
            >
              <div className="flex justify-between items-start">
                ...
                <span className="text-[9px] font-bold uppercase tracking-widest text-[#075056]/30 group-hover:text-[#FF5B04]">{report.type}</span>
              </div>
              <div className="space-y-1">
                <h4 className="text-sm font-medium text-[#075056] group-hover:text-[#075056]">{report.title}</h4>
                <p className="text-[10px] text-[#075056]/40">{report.date} • {report.size}</p>
              </div>
              <button 
                onClick={() => {
